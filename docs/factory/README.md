...
4. 人类只做标准 GitHub Review：通过或要求修改；GitHub 自动绑定方案提交，Agent 在同一 PR 重做
   Spec 或进入实现。
...
同一 Draft PR、Review、`factory.json`、分支和 Checks 恢复到正确步骤。

当前项目关闭 `autonomous`、自动合并和自动发布。`animal-exhibit-v1` 处于监督模式，同类动物展品
需要在 GitHub PR Review 统一 Spec；最终合并就是产品验收。连续 3 次干净执行后才具备晋级条件。
