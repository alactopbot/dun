...
- **变化：** 需要人工方案确认时，方案提交后立即创建该需求唯一 Draft PR。技术方案、例外授权、
  产品验收和合并都复用它，并用可信人类身份与提交 SHA 留下结构化证据；Agent 会话不再承载批准。
...
- **变化：** 既有测试只能在 GitHub 已批准技术方案中预先授权，或由同一 Draft PR 上的专门 Gate
  批准时修改；聊天不能提供该授权。
