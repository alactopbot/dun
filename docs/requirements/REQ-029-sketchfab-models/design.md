...
把默认分支现有的三角龙和剑龙运行时模型替换为来自 Sketchfab、允许下载、修改、商业使用和随网页再分发的固定模型，同时保留已经交付的展品内容、安静交互、性能、无障碍和静态回退。

完成时：

- `/exhibits/triceratops` 和 `/exhibits/stegosaurus` 使用本 Spec 固定的两个 Sketchfab 模型，不再把 Quaternius 模型作为运行时模型；
...
- 两个展品在支持的浏览器和设备上继续满足既有旋转、缩放、重置、键盘、响应式、减少动态效果、失败回退和性能约束；
- `./.factory/scripts/gates.sh deep` 为 GREEN，当前 PR head 通过独立验证。

## 2. 范围决定

本需求只替换默认分支当前已发布的三角龙与剑龙模型。开放 PR 中尚未进入 `main` 的动物不属于本需求；它们应在自己的 Issue、分支和 PR 中保持独立授权与证据。
...
人类把本 Draft PR 设为 Ready，即只批准以下两个固定 UID 进入下载与审查：
...
两页的许可链接均指向 [Creative Commons Attribution 4.0](https://creativecommons.org/licenses/by/4.0/)。CC BY 4.0 允许复制、再分发与改编（包括商业用途），条件是提供适当署名、许可证链接并说明修改。DUN 必须把作者与 Sketchfab 来源归属随模型、credits 和可再分发构建产物一起保留。
...
实现开始后必须通过选定详情页的官方 Download 入口取得归档，并在入库前保存页面可见的作者、UID、许可和下载日期证据。不得读取浏览器 cookie、local storage、密码或 token，也不得把账号凭据提交到仓库。
...
- 三角龙复核四足体态、三只面角和颈盾；剑龙复核四足体态、背板序列和尾刺。模型不是科学来源，不根据模型颜色、姿态或细节改写儿童事实。
...
- `content/exhibits/{triceratops,stegosaurus}/**`：更新模型 provenance、evidence、manifest，必要时更新 presentation；属于承重路径。
- `public/museum/animals/{triceratops,stegosaurus}/**`：替换两个 GLB 和哈希绑定的 poster/thumbnail；必要时才重做背景；属于承重路径。
...
- 两个展品的双语事实、来源映射、观察问题、照护者提示、导航、背景归属和返回路径不丢失。
...
- 代码继续使用 AGPL-3.0；DUN 原创背景和派生图像继续使用 CC BY-SA 4.0；两个第三方模型及其修改保留 CC BY 4.0。

## 9. 实现顺序

1. 先增加失败测试，证明当前 manifest/credits 仍指向 Quaternius、当前 poster 与旧模型哈希绑定。
2. 从两个固定 Sketchfab 页面下载归档并保存许可、作者、UID、原始文件和 SHA-256 证据；先做权利与内容审查，失败即转回 Draft。
3. 在隔离 Blender 流水线中规范化、清理和优化两只模型；运行现有 inspect、预算和 Khronos Validator。
...
6. 完成自动测试、六个已批准视口、200% 缩放、减少动态效果、真实中档手机性能和来源面板检查。
...
- 只允许本 Spec 固定的两个 UID；仍引用 Quaternius、未知 Sketchfab UID、NC/ND/Editorial/view-only 或品牌提取来源时失败。
...
- 来源面板对两只动物分别输出正确标题、作者、Sketchfab 链接、CC BY 4.0、修改说明与运行时哈希，不再出现 Quaternius 运行时归属。
- 两个既有路由、首页入口、事实卡片、来源链接、导航、键盘控制、失败回退、reduced motion 和资源销毁测试保持通过。
...
- 在真实中档手机记录两个 GLB 的字节、解析/首帧时间、三角形、draw calls、纹理数/最大尺寸、平均帧率和内存观察；平均帧率目标不低于 30fps。
