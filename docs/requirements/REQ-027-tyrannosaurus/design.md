...
- 不承诺旁白；本次没有音频资产和音频控制。
- 不把真机帧率、解析时间或内存采样作为单只霸王龙展品的交付门槛；移动端只要求响应式、完整内容和失败降级可用。
...
- `docs/MUSEUM_TECHNICAL_PLAN.md`：明确桌面 Web 是正式性能基准；移动端保持响应式和静态降级可用，真机性能采样不作为每只动物的硬门。
...
本需求明确包含 `content/exhibits/**`、`public/museum/**`、`app/**`、`tests/**` 和 `docs/MUSEUM_TECHNICAL_PLAN.md` 承重路径，因此实现使用 Deep Gate。不修改 `package.json`、lockfile、许可证文件、Factory/Agent/Codex 治理或发布配置。
...
在 360×800、390×844、768×1024、1366×768、1920×1080，以及低高度横屏视口检查 poster、模型首帧、旋转后的头/短前肢/脚/长尾、来源区、加载失败和 reduced motion。确认轮廓未被 UI 裁切、脚与地面接触、横竖背景独立、双语不溢出、键盘顺序合理，模型动作安静且没有咆哮或攻击姿态。

发布证据记录可确定复现的 GLB 字节数、三角形、draw calls、纹理数量与最大尺寸，并在受支持桌面浏览器确认模型能加载、显示首帧和响应旋转、缩放及重置，过程中没有明显长时间冻结或控制台错误。移动端通过上述响应式视口验证正文、导航、控制和 poster/失败降级，不要求真机帧率、解析时间或内存采样。
...
- 五个基准视口与低高度横屏、200% 缩放、静态素材预算、桌面浏览器性能观察、移动端响应式与失败降级、自动测试、Deep Gate 和独立验证全部通过。
