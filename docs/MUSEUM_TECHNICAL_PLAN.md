...
首版采用 Meshopt + WebP 纹理。KTX2 能进一步降低 GPU 纹理内存，但会增加转码器、兼容性和构建复杂度，应在受支持桌面浏览器的可复现性能数据证明有必要后再引入。
...
不能盲目接受 optimize 默认结果。处理后必须在 Three.js、Khronos Viewer 和受支持桌面浏览器上检查：
...
移动端通过响应式视口、静态 poster 和失败降级检查保证内容可阅读、可导航；单只动物发布不要求真机
帧率、解析时间或内存采样。只有后续需求明确改变公共展示器、运行时预算或正式支持范围时，才在对应
Spec 中增加针对性的设备性能验证。
...
以桌面 Web 为正式性能基准；移动端保持响应式和降级可用，不作为单只动物发布的性能采样门槛：

- poster 的 LCP 目标小于 2.5 秒。
- 首只 GLB 下载和解析期间提供可理解的显示进度。
- 受支持桌面浏览器中的模型显示目标为 60fps。
- 首屏主线程不因同步解析模型长时间冻结。
- Canvas DPR 有上限。
...
- 移动端视口不横向溢出，正文、导航和控制保持可用；性能不足、WebGL 不可用或模型加载失败时保留完整静态展品。

每只动物发布证据应包含以下可确定复现的静态预算指标：
...
桌面浏览器 QA 还应确认模型能完成加载、显示首帧并响应旋转、缩放和重置，过程中无明显长时间冻结
或控制台错误。解析/首帧计时、帧率和 GPU/页面内存属于公共展示器或版本级性能调查证据；除非对应
Spec 明确改变相关运行时行为或接近素材硬上限，否则不作为新增单只动物的交付硬门。
...
- 受支持桌面浏览器满足素材预算和交互性能目标。
- 移动端保持响应式，低性能、无 WebGL 或模型失败时仍可阅读完整展品并完成导航。
- 所有资产满足预算或有批准记录。
