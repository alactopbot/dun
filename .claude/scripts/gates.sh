...
if [ "$LEVEL" = "deep" ]; then
  case "$STACK" in
    node)
      if pkg_has audit; then run audit $PMRUN audit
      else run audit $PM audit --audit-level=high; fi
  ...
  printf '\n=== gate: architecture ===\n'
  ARCH_FAIL=0
  if grep -RniE 'google-analytics|googletagmanager|mixpanel|segment\.com|amplitude|hotjar|facebook[-_]?pixel' \
      app worker package.json --include='*.ts' --include='*.tsx' --include='*.js' --include='*.json' 2>/dev/null; then
    c_red "architecture: behavioral tracking or advertising integration detected"
    ARCH_FAIL=1
  fi
  if grep -RniE '\bautoplay\b|\bautoPlay\b' app --include='*.ts' --include='*.tsx' --include='*.js' 2>/dev/null; then
    c_red "architecture: autoplay is forbidden for DUN"
    ARCH_FAIL=1
  fi
