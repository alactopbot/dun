...
description: 在统一 Spec 已通过 Review 或由成熟 Pattern 自动通过后，恢复同一需求分支与 Draft PR，完成实现、机器 Gate 和独立验证，最终等待人类合并。
---

# Factory 实现

一次运行推进一个完整需求。读取契约、章程、项目配置、Pattern、Issue、`design.md`、`factory.json`、
唯一 Draft PR、最新可信 Review 和交接后再行动。Agent 会话不是状态来源。
...
- 同一 Issue 只有一个开放 PR；
- `bootstrap`/`supervised` 的最新可信 Review 是 `APPROVED` 或首行 `方案通过`；
- Review 提交之后没有 Spec、机读范围、Pattern 或项目策略漂移；
...
变化时，先更新 `design.md` 与 `factory.json`，回到同一 PR 重新 Spec Review。
...
启动全新上下文验证器，冷读 Issue、Spec、机读范围、Review、完整 diff、测试与 GitHub 状态。
