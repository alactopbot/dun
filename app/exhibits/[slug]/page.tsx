...
export default async function ExhibitPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const licenseLabel = model.license === "CC-BY-4.0" ? "CC BY 4.0" : "CC BY-SA 4.0";
  const licenseName = model.license === "CC-BY-4.0" ? "Creative Commons Attribution 4.0" : "Creative Commons Attribution-ShareAlike 4.0";
  return <main className={styles.page} id="content">
    ...
    <article className={styles.exhibit} id="exhibit">
      ...
      <details className={styles.sourcePanel} data-source-credit><summary>来源与创作说明</summary>
        ...
        <p>模型：{model.creator}{model.source ? <> · <a href={model.source} target="_blank" rel="noreferrer">{model.title}（Sketchfab）</a></> : <> · {model.title}</>} · {licenseLabel}。{model.modifications}</p>
        <p>署名：{model.attribution}。许可证：<a href={model.licenseUrl} target="_blank" rel="noreferrer">{licenseName}</a>。</p>
