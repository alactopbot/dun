const rooms = [
  { icon: "◒", title: "大地展厅", subtitle: "LAND", text: "从三角龙开始，观察脚印、牙齿和骨骼。" },
    ...
    <main>
      ...
      <section className="rooms section" id="museum">
        ...
        <div className="room-grid">
          {rooms.map((room) => <article className="room-card" key={room.title}><div className="room-icon" aria-hidden="true">{room.icon}</div><p>{room.subtitle}</p><h3>{room.title}</h3><span>{room.text}</span><span className="coming">逐步开放</span></article>)}
        </div>
      </section>
